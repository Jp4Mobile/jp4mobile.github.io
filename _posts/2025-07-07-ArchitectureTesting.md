...
Inexhaustive testing would be working from inaccurate initial state, and things would continue to fail from that point.
...
While those challenges exist, I don't think that's enough to stop me from continuing with TCA for `TaskManager` going forward. I really like the clarity and the expansiveness of the library. Especially for new SwiftUI projects.
